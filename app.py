...
app.config['MYSQL_PASSWORD'] = 'omer2002'
...
def Football():
    ...
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute("SELECT id, name FROM Teams WHERE type='Football' LIMIT 14")
...
def Basketball():
    ...
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute('SELECT id, name FROM Teams WHERE type = "Basketball" LIMIT 12')
    teams = cursor.fetchall()
    cursor.close()
